    ...
    <v-container fluid fill-height id="router"><v-row><v-col cols="12" md="10" offset-md="1" lg="10" offset-lg="1" xl="6" offset-xl="3">
        ...
        <v-card class="pa-4 d-flex flex-column" dark>
            ...
            <v-card-text>
                ...
                label="Nazwa quizu"
                :rules="[v => !!v || 'Nazwa quizu jest wymagana.', v => v.length >= 5 || 'Nazwa quizu za krótka.', v => v.length <=128 || 'Nazwa quizu za długa']"
                ></v-text-field>
            ...
            <draggable v-model="questions" group="questions" @start="drag=true" @end="drag=false" handle=".handleQuestion">
            <v-card dark class="mt-4 pb-6" v-for="(question, questionIndex) in questions" :key="questionIndex" @mouseover="m_index.question=questionIndex" @mouseleave="m_index.question=-1">

            <div class="questionValidation" :class="(question.valid ? '' : 'error')">
                <div class="controls" :style="'visibility: ' + ((m_index.question==questionIndex) ? 'visible;' : 'hidden;')">
                    <v-card-actions class="justify-center handleQuestion ma-0 pa-0">
                        <v-icon>mdi-drag-horizontal</v-icon>
                    </v-card-actions>

                    <v-col class="text-right pa-0" style="margin-top: -24px">
                        <v-icon @click="questions.splice(questionIndex, 1)">mdi-close</v-icon>
                    </v-col>
                </div>
            </div>
            ...
            <v-card-text class="pt-0">
                        ...
                        <draggable v-model="question.answers" group="answers"  @start="drag=true" @end="drag=false" handle=".handleAnswers">
                            <v-flex @mouseover="m_index.answer=answerIndex" @mouseleave="m_index.answer=-1"  v-for="(answer, answerIndex) in question.answers" :key="answerIndex">
                            <v-text-field
                            v-model="answer.text"
                            label="Odpowiedź"
                            :value="answer.text"
                            ...
                            class="ml-8"
                            hide-details
                            @change="(answer.text == '' ? questions[questionIndex].answers.splice(answerIndex, 1) : 'none') "
                            >
                            <template v-slot:prepend>
                                <v-icon class="handleAnswers" @click="() => (answer.correct = !answer.correct)" :color="answer.correct ? 'success' : 'error'">{{answer.correct ? 'mdi-checkbox-marked-circle-outline' : 'mdi-checkbox-blank-circle-outline'}}</v-icon>
                            </template>
                            <template v-slot:append-outer>
                                <v-icon v-if="m_index.answer==answerIndex&&m_index.question==questionIndex" @click="questions[questionIndex].answers.splice(answerIndex, 1)">mdi-close</v-icon>
        ...
        <v-col><v-btn class="secondary" @click="questions.push({ text: '', answers: [{text: '', correct: false}], valid: true})"><v-icon>mdi-plus</v-icon></v-btn></v-col>
        </draggable>
  ...
  export default {
        ...
        questions: {
            ...
            handler(){
                this.questions.forEach(q => {
                    if(q.answers.length == 0 || q.answers[q.answers.length - 1].text !== '') q.answers.push({text: '', correct: false});
                })
    ...
    methods: {
      async submitForm () {
        ...
        if(wrong.length !== 0){
            this.$store.commit('error', (wrong.length > 1 ? "Pytania " + wrong.join(', ') + " są niepoprawne." : "Pytanie " + wrong[0] + " jest niepoprawne."))
            return false;
        }
