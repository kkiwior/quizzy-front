<template>
    <v-container fluid fill-height><v-row><v-col cols="12" md="10" offset-md="1" lg="10" offset-lg="1" xl="6" offset-xl="3">
        <v-card v-if="quiz.id && game.id == 0" dark class="pa-4 d-flex flex-column">
            <v-card flat>
            <v-card class="d-flex flex-wrap flex-row mt-2 flex-align-center justify-center" flat>
                <v-card class="align-self-center text-center" flat>
                    <v-img v-if="quiz.thumbnail == null" class="mt-2 elevation-4" height="128" width="256" src="/images/quizthumbnail.jpg" eager></v-img>
                    <v-img v-else class="mt-2 elevation-4" height="128" width="256" :src="quiz.thumbnail" eager></v-img>
                </v-card>
            ...
            <v-card style="min-width:80% flex-align-center justify-center" flat>
                ...
                >
                    ...
                    <template v-slot:item.user="{ item }">
                        <v-avatar v-if="item.user.avatar === null" size="40" rounded="circle" class="mr-2 elevation-3" color="secondary">{{item.user.name[0]}}</v-avatar>
                        <v-avatar v-else size="40" rounded="circle" class="mr-2 elevation-3"><v-img :src="item.user.avatar" :alt="item.user.name"></v-img></v-avatar>{{ item.user.name }}
                    </template>
                </v-data-table>
            </v-card>
            ...
            <v-card class="d-flex flex-row mt-4 flex-align-center justify-center" flat>
                    ...
                    <v-card class="ml-2 mr-2" flat>
                        <v-avatar v-if="quiz.creator.avatar === null" size="40" rounded="circle" class="elevation-3" color="secondary">{{ quiz.creator.name[0] }}</v-avatar>
                        <v-avatar v-else size="40" rounded="circle" class="elevation-3"><v-img :src="quiz.creator.avatar" :alt="quiz.creator.name"></v-img></v-avatar>
                    </v-card>
        ...
        <v-container v-else>
            ...
            <v-progress-linear v-if="quiz.time" v-model="timer" height="25" class="mt-2 mb-2"><strong>{{ Math.ceil(timer / 100 * this.quiz.time) }}</strong></v-progress-linear>

            <v-row id="questions">
                <v-col cols="6" v-for="(answer, aid) in question.answers" :key="aid">
                    <v-btn class="pa-4 text-none" style="width: 100%;" dark :disabled="button" @click="sendAnswer(answer.id)" large>{{answer.text}}</v-btn>    
                </v-col>
            </v-row>
        </v-container>
      ...
      >
          <v-card>
            <v-card-text class="pa-4">
              Rozwiązałeś quiz! Udało ci się odpowiedzieć na {{ this.game.points }} z {{ this.quiz.questions_count }} pytań.
            </v-card-text>
  ...
  export default {
    ...
    methods: {
        ...
        async sendAnswer(id){
            this.button = true;
            clearInterval(this.interval);
            await axios.post("game/" + this.game.id, { answer: id })
                .then(response => {
                    if(id != 0 && !response.data.correct.includes(id)) document.querySelector("#questions div:nth-child(" + (id) +") button").classList.add("error");
                    response.data.correct.forEach(c => {
                        document.querySelector("#questions div:nth-child(" + (c) +") button").classList.add("success");
                    });

                    if(response.data.finish === undefined)
                    setTimeout(()=>{
                        document.querySelectorAll("#questions div button").forEach(el => {
        ...
        async getRanking(){
            this.$store.commit('progressbar', true);
            await axios.get("quiz/ranking/" + this.$route.params.id)
                .then(response => {
                    let ranking = [];
                    let place = 0;
                    response.data.ranking.forEach(el=>{
                        ranking.push({ place: (++place + '.'), user: el.user ?? {name: 'Anonymous', avatar: null}, score: el.correctAnswers });
                    });
